...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (msg) => {
        ...
        if (data.type === 'toggleGame') {
            ...
            // ================================================================ PASS PLAY
            // ** start a pass play by getting ball x,y
        } else if (data.type === 'startpass') {
            console.log('=========================');
            console.log('>>> PASS PLAY STARTED');
            console.log('..playStopped');
            console.log('=========================');
            game.playState = 'pass';
            game.gameRunning = false;
            game.playClockRunning = false;
            ...
            console.log('Server: Pass initiated game.ball', game.ball);

            broadcastState(game);
        } else if (data.type === 'passThrown') {
            console.log('=========================');
            console.log('>>> PASS THRWON');
            console.log('..');
            console.log('=========================');
            game.playState = 'passThrown';
            ...
            // };
            broadcastState(game);
            console.log('Server: Pass initiated', game.ball);
...
function tackleMade(tackledPlayerID, tacklerID, frontEdgeX) {
    ...
    game.currentPlay = 'end';
    game.playState = 'tackle'; // Reset play state
    ...
    if (game.playState === 'kickoff') {
        ...
        game.playClockRunning = true; // Start the play clock
        ...
        message = { text: `Runback by ${tackledPlayer?.name} tackled by ${tackler.name}`, type: "info" };
...
function handlePass() {
    if (!gameState.ball || !gameState.ball.isMoving) return;

    // Move ball
    gameState.ball.x += gameState.ball.vx;
    gameState.ball.y += gameState.ball.vy;
    gameState.ball.vx *= 0.97;
    gameState.ball.vy *= 0.97;

    const speed = Math.hypot(gameState.ball.vx, gameState.ball.vy);

    // Interception logic
    const defenders = gameState.isTeam1Offense
        ? gameState.players.filter(p => !p.pid.includes('-h-'))
        : gameState.players.filter(p => p.pid.includes('-h-'));
    for (let defender of defenders) {
        const dx = gameState.ball.x - defender.x;
        const dy = gameState.ball.y - defender.y;
        ...
        if (dist < 30 && speed < 5 && speed > 0.5) {
            ...
            if (Math.random() < interceptionChance) {
                ...
                gameState.isTeam1Offense = !gameState.isTeam1Offense;
                gameState.scoringBoundary = gameState.isTeam1Offense
                    ? (gameState.isRightTeam ? 1920 - 120 : 120) // Assume canvas.width = 1920
                    : (gameState.isRightTeam ? 120 : 1920 - 120);
                gameState.down = 1;
                gameState.yardsToFirst = 10;
                gameState.PlayState = "complete";
                console.log('Server: Intercepted by:', defender.label);
                broadcastGameState();
                return;
            }
        }
    ...
    if (gameState.selectedReceiver) {
        const dx = gameState.ball.x - gameState.selectedReceiver.x;
        const dy = gameState.ball.y - gameState.selectedReceiver.y;
        const dist = Math.hypot(dx, dy);

        if (dist < 30 && speed < 10 && speed > 0.5) {
            gameState.selectedReceiver.hb = true;
            gameState.ball = null;
            gameState.selectedReceiver = null;
            gameState.PlayState = "complete";
            console.log('Server: Catch successful!');
            broadcastGameState();
            return;
        }
    }
    ...
    if (speed < 0.1) {
        ...
        gameState.selectedReceiver = null;
        gameState.PlayState = "complete";
        console.log('Server: Incomplete pass.');
...
function broadcastState(game) {
    const payload = {
        ...
        qtr: game.qtr,
        down: game.down,
        passMode: game.passMode,
        b: game.ball ? {
            x: Number((game.ball.x || 0).toFixed(2)),
            y: Number((game.ball.y || 0).toFixed(2)),
            vx: Number((game.ball.vx || 0).toFixed(2)),
            vy: Number((game.ball.vy || 0).toFixed(2)),
            // r: Number((game.ball.rotation || 0).toFixed(2)),
            // t: (game.ball.trail || []).map(t => ({
            //     x: Number((t.x || 0).toFixed(2)),
            //     y: Number((t.y || 0).toFixed(2))
            // })),
            m: !!game.ball.isMoving // Moving flag
        }
    };
    // console.log('===============================');
